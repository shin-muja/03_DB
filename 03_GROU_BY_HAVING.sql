...
-- EMPLOYEE 테이블에서 부서코드, 쿠서별 급여 합조회
...
FROM EMPLOYEE; -- SQL Error [937] [42000]: ORA-00937: 단일 그룹의 그룹 함수가 아닙니다

-- 이렇게 해결
SELECT NVL(DEPT_CODE, '유령부서'), SUM(SALARY)
FROM EMPLOYEE
GROUP BY DEPT_CODE;
-- DEPT_CODE 컬럼을 그룹으로 묶어, 그 그룹의 급여 합계(SUM(SALARY))를 구함

-- EMPLOYEE 테이블에서 (FROM 절)
-- 직급코드가 같은 사람의 ( 지급을 그룹으로 묶기 : JOB_CODE)
-- 직급코드, 급여 평균, 인원 수를 (SELECT 절)
-- 직급코드 오름차순으로 조회 (ORDER BY 절)
SELECT JOB_CODE, ROUND(AVG(SALARY), -1), COUNT(*)
FROM EMPLOYEE
GROUP BY JOB_CODE
ORDER BY JOB_CODE;
...
ORDER BY "인원 수" DESC; -- 별칭 사용 O( SELECT 절 해석 완료)
...
--> WHERE 절은 각 컬럽 값에 대한 조건
--> HAVING 절은 그룹에 대한 조건

-- EMPLOYEE 테이블에서 (FROM 절)
-- 부서코드가 D5, D6인 부서의 ( WHERE 절 -> GROUP BY 절)
-- 부서코드, 평균급여, 인원 수 조회 (SELECT 절)
SELECT DEPT_CODE, ROUND(AVG(SALARY), -1) "평균 급여", COUNT(*) "인원 수"
...
-- EMPLOYEE 테이블에서
-- 부서별 평균 급여가 3 백만원 이상인 부서의
-- 부서코드, 평균급여 조회
-- 부서코드 오름차순
SELECT DEPT_CODE, ROUND(AVG(SALARY), -1) "평균 급여"
-- WHERE SALATY >= 3000000--> 한사람의 급여가 3백만원 이상이라는 조건(요규사항 X)
FROM EMPLOYEE
GROUP BY DEPT_CODE 
HAVING AVG(SALARY) >= 3000000 --> DEPT_CODE 그룹 중급여 평균이 3백만원 이상인 그룹만 조회
ORDER BY DEPT_CODE
