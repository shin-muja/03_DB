...
-- 3) FULL [OUTER] JOIN : 합치기에 사용한 두 테이블에 가진
-- 모든 행을 결과에 포함
...
WHERE DEPT_CODE = DEPT_ID -- EMPLOYRR + DEPARTMENT JOIN
AND LOCATION_ID = LOCAL_CODE; -- (EMPLOYRR + DEPARTMENT) + LOCATION JOIN
...
AND TO_DATE(SUBSTR(EMP_NO, 1, 2), 'RR') BETWEEN '1970-01-01' AND '1979-12-31';
...
-- (연봉에 보너스포인트를 적용하시오.)
SELECT EMP_NAME, JOB_NAME, SALARY, NVL2(BONUS, SALARY * 12 * (BONUS + 1), SALARY * 12)
FROM EMPLOYEE
JOIN JOB USING(JOB_CODE)
...
SELECT EMP_NAME, DEPT_TITLE , LOCAL_NAME, DECODE(NATIONAL_CODE, 'KO', '한국', 'JP', '일본') 국가명
FROM EMPLOYEE
JOIN DEPARTMENT ON( DEPT_CODE = DEPT_ID )
JOIN LOCATION ON( LOCATION_ID = LOCAL_CODE )
