...
ALTER TABLE DEPT_COPY
MODIFY DEPT_TITLE NULL; -- DEPT_TITLE 컬럼에 NULL 비허용
...
-- "KH_SDK"."DEPT_COPY"."DEPT_ID" 열에 대한 값이 너무 큼(실제: 3, 최대값: 2)
-- DEPT_ID의 타입이 CAHR(2)라 고정길이 2BYTE까지만 저장가능해서 생기는 에러
-- 영어 + 숫자 2즐자까지 저장가능
--> D10은 CHAR(30)에 들어올 수 있음
...
SELECT * FROM DEPT_COPY;
--> DEFAULT 값을 변경했다고 해서 기존 삽입된 전 DEFAULT값은 변하지 않음
...
-- 부모인 TB1 테이블을 삭제할겨 할 때

DROP TABLE TB1;
-- SQL Error [2449] [72000]:
-- ORA-02449: 외래 키에 의해 참조되는 고유/기본 키가 테이블에 있습니다
--> 해결방법
-- 1) 자식, 부모테이블 순서로 삭제할 수 있다
-- 2) ALTER를 이요해서 자식테이블 FK 제약 조건 삭제 후 TB1 삭제
...
SELECT * FROM TB1; -- 삭제되서 확인 안됨
SELECT * FROM TB2; -- FK 제약조건도 삭제됨 그외는 X
...
SELECT * FROM TB2;
-- 롤백 안된다... 위에서 DDL 구문 중 ALTER를 사용해서 그 시점에 COMMIT 됨
