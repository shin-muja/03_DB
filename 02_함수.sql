...
-- 문자열을 5번째 문자부터 검색하여 2번째 B 위치 조회
...
SELECT EMP_NAME, SUBSTR(EMAIL, 1, INSTR(EMAIL, '@') - 1 ) AS "찾은 ID"
FROM EMPLOYEE;
...
SELECT TRIM('                     H EL L O          ') -- 기본값으로 양쪽 공백만 제거한 값이 나옴 'H EL L O'
FROM DUAL;
...
SELECT '절대 값 같음' FROM DUAL
WHERE ABS(10) = ABS(-10); -- WHERE 절 함수 작성 가능

-- MOD( 숫자 | 컬럼명, 숫자 | 컬럼명) : 나머지 값 반환

-- EMPLOYEE 테이블에서 사원의 월급을 100만으로 나눴을 때 나머지 조회
SELECT EMP_NAME, SALARY, MOD(SALARY, 1000000) "100만 나눈 나머지"
FROM EMPLOYEE;

-- EMPLOTEE 테이블에서 사번이 짝수인 사원의 사번, 이름 조회
...
-- ROUND(숫자 | 컬럼명[, 소수점위치 ]) : 반올림, 기본값 소수점 첫번째 자리

SELECT ROUND(123.456/*, 0*/) FROM DUAL; -- 123, 소수점 첫 번째 자리에서 반올림 

SELECT ROUND(123.456, 1) FROM DUAL; 
-- 123.5 소수점 첫 번째 자리까지 표기 - 소수점 2번째 자리에서 반올림
...
-- MONTHS_BETWEEN(날짜, 날짜) : 두 날짝의 개월 수 차이 반환
...
SELECT EMP_ID, HIRE_DATE, ABS(CEIL(MONTHS_BETWEEN(SYSDATE, HIRE_DATE))) "근무한 개월수",
ABS(CEIL(MONTHS_BETWEEN(SYSDATE, HIRE_DATE) / 12)) || '년차' "근무 년차"
FROM EMPLOYEE;

-- '||' : 연결 연산자( 문자열 이어쓰기 )

-- ADD_MONTHS(날짜, 숫자) : 앞에 날짜에 숫자만큼의 개월 수를 더함( 숫자에 음수도 가능 )
...
SELECT TO_CHAR(1234, '90999') FROM DUAL; -- ' 1234'
...
SELECT TO_CHAR(SYSDATE, 'YYYY/MM/DD HH24:MI:SS DAY') "오늘" FROM DUAL;

-- 10/08 (화)
SELECT TO_CHAR(SYSDATE, 'MM/DD') || ' (' || TO_CHAR(SYSDATE, 'DY') || ')' "장난"
FROM DUAL;

-- 2024년 10월 08일 (화)
SELECT TO_CHAR(SYSDATE, 'YYYY"년" MM"월" DD"일" (DY)') "오늘"
...
SELECT TO_DATE('400505', 'YYMMDD') FROM DUAL; -- 2040-05-05 00:00:00.000
...
-- 1) 주민번호에서 '-' 앞글자까지 추출
-- 2) 추출한 생년월일을 TO_DATE타입으로 변경 -> RR패턴을 이용하여 1900년도로 바꿈
-- 3) TO_CHAR를 이요해서 문자열로 변환 -> 1965SUS 10월 08일
...
SELECT EMP_NAME, SALARY, NVL(BONUS, 0) "BONUS", NVL(SALARY * BONUS, 0 )
FROM EMPLOYEE;

-- NVL2(컬럼명, 바꿀값1, 바꿀값2)
-- 해당 컬럼의 값이 있으면 바꿀갑 1로 변경,
-- NULL 이면 바꿀값2 로 변경
...
-- 직원의 성별 구하기
SELECT EMP_NAME, DECODE(SUBSTR(EMP_NO, INSTR(EMP_NO, '-') + 1, 1), '1', '남성', '2', '여성') 성별
FROM EMPLOYEE;
...
-- SUM(숫자가 기록된 컬럼명) : 합계
-- 모든 직원의 급여 합 조회
SELECT SUM(SALARY) "모든 사원들의 급여 합"
FROM EMPLOYEE; -- 70096240

-- AVG(숫자가 기록된 컬럼명) : 평균
-- 전 직원 급여 평균
SELECT  ROUND (AVG(SALARY), -2)  FROM EMPLOYEE; --3047700


-- 부서코드가 'D9'인 사원들의 급여 합, 평균
/* 3 */ SELECT SUM(SALARY), ROUND(AVG(SALARY), -1) 
/* 1 */ FROM EMPLOYEE
...
-- 급여 최소값, 가장빠른 입사일, 알파뱃 순서가 가장빠른 이메일 조회
SELECT MIN(SALARY), MIN(HIRE_DATE), MIN(EMAIL)
FROM EMPLOYEE;

-- 급여 최대값, 가장 늦은 입사일, 알파뱃 순서가 가장 느린 이메일 조회
...
SELECT COUNT(*) FROM EMPLOYEE; -- EMPLOYEE 테이블의 행의 가수
