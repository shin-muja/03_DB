...
-- 산술연산은 숫자 타입(NUMBER타입)만 가능하다
-- 하지만 숫자로만 이루어진 문자 타입 테이블은 연산이 가능하다

SELECT '같음'
FROM DUAL 
WHERE 1 = '1';
-- WHERE : 조건절, '=' : 같음
-- 문자열안DP 숫자만 있을 시 자동 형변환으로 산술연산 가능
 
SELECT EMP_ID - 199 FROM EMPLOYEE;
...
-- DAUL(DUmmy tAbLe) 테이블 : 가짜 테이블(임시 조회용 테이블)
...
-- JAVA 리터럴 : 값 자체를 의미
-- DB 리터럴 : 읨의로 지정한 값을 기존테이블에 존재하는 값처러 사용하는 것
...
SELECT DISTINCT
DEPT_CODE, JOB_CODE FROM EMPLOYEE;

-- 3. SELECT 절 : SELECT 컬럼명
-- 1. FROM 절 : FROM 테이블명
-- 2. WHERE 절(조건절) : WHERE 컬럼명 연산자 값
-- ORDER BY컬럼명 | 별칭 | 컬럼 순서 [ASC | DESC] [NULLS FIRST | LAST]
--								오름차순|내림차순 생략시 기본으로 오름차순

-- EMPLOYEE 테이블에서 급여가 3백만원 초과인 사원의
-- 사번, 이름, 급여, 부서코드 조회해라
SELECT EMP_ID, EMP_NAME, SALARY, DEPT_CODE
FROM EMPLOYEE
WHERE EMPLOYEE.SALARY > 3000000;
...
-- EMPLOYEE 테이브렝서 입사일이 1990-01-01 ~ 1999-12-31 사이인 직원의
-- 이름, 입사일 조회
...
-- EMPLOYEE 테이블에서
-- 이메일'_' 앞이 4글자 이면서
-- 부서코드가 'D9' 또는 'D6' 이고 --> AND 가 OR 보다 우선순위가 높다, () 사용가능
 ...
 * 2. 연결 연산자 ( || ) 
 * 3. 비교 연산자 ( > < >= <= = != <> )
 * 4. IS NULL / IS NOT NULL, IN / NOT IN
...
OR DEPT_CODE IS NULL; -- 부서 코드가 없는 2명 포함 14명
...
-- EMPLOYEE 테이블(FROM 절) 급여 오름 차순으로 ( ORDER BY절 )
-- 사번, 이름, 급여 조회 ( SELECT 절)

SELECT EMP_ID, EMP_NAME, SALARY
FROM EMPLOYEE
ORDER BY SALARY ASC; -- ASC(오름차순)가 기본값 / DESC(내림차순)
